...
export default function FleetPreview({ vehicles = [] }: { vehicles?: any[] }) {
            ...
            <AnimatedSection key={vehicle._id || vehicle.name} delay={index * 0.1}>
              <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden hover:shadow-2xl transition-all flex flex-col h-full group">
                ...
                <div className="relative bg-slate-900 h-52 overflow-hidden flex items-center justify-center">
                  <img
                    src={vehicle.imageUrl || "https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=600&q=80"}
                    alt={vehicle.name}
                  ...
                  {vehicle.subtitle && (
                    <div
                      className={`absolute top-4 left-4 bg-saffron-600 text-white font-bold text-xs uppercase px-3 py-1.5 rounded-full shadow`}
                    >
                      {vehicle.subtitle}
                    </div>
                  )}
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <div className="grid grid-cols-2 gap-4 my-5 bg-white p-3.5 rounded-xl border border-slate-100 text-xs text-slate-600">
                    ...
                    <div>
                      ...
                      <span className="font-bold text-slate-900 flex items-center gap-1">
                        <Snowflake className="w-3.5 h-3.5 text-sky-500 animate-spin" style={{ animationDuration: "4s" }} />
                        {vehicle.acType}
                      </span>
                    </div>
                  </div>
