...
import { buildContactMessage, sendBookingInquiry } from "@/lib/whatsapp";
...
export default function ContactSection() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      const messageText = buildContactMessage({
        name: data.name,
        phone: data.phone,
        message: data.message,
      });

      // Submit booking log in non-blocking manner and open WhatsApp
      await sendBookingInquiry({
        name: data.name,
        phone: data.phone,
        vehicle: "sedan", // General fallback choice
          ...
          <AnimatedSection className="lg:col-span-5">
            <div className="space-y-4">
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                Have a question or custom tour package requirement? Give us a call or drop by our
                main booking office in Lashkar, Gwalior.
              </p>
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="block text-slate-200 text-xs mt-0.5">{BUSINESS.email}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-800 p-5 sm:p-6 rounded-xl border border-slate-700 space-y-3">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-saffron-600 hover:bg-saffron-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-saffron-600/15 transition-all flex items-center justify-center gap-2 touch-target cursor-pointer"
                >
                  {isSubmitting ? (
